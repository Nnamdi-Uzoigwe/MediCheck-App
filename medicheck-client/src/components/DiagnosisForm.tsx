...
//     // Navigate to diagnosis page with result data
...
export default function DiagnosisForm() {
  ...
  const checkEmergencySymptoms = () => {
    ...
    const highPainScale = formData.coreSymptoms.painScale && formData.coreSymptoms.painScale >= 8;
    
    if (emergencySymptoms.length > 0 || highPainScale) {
      return {
        isEmergency: true,
        message: "⚠️ Based on your symptoms, you should seek immediate medical attention. Please contact emergency services or visit the nearest emergency room."
      };
    }
...
//     try {
//       // Update the endpoint to match your new backend route
//       console.log("Token in sessionStorage:", sessionStorage.getItem('token'));
//       const response = await fetch("https://medicheck-app-3.onrender.com/api/symptoms", {
//         method: "POST",
//         headers: { 
...
//       const { diagnosisId, status } = await response.json();

//       // Navigate to diagnosis page with just the ID
//       navigate("/diagnosis", { 
      ...
      <div className="bg-white w-full max-w-2xl p-4 rounded-[10px] border-2 border-gray-400">
        ...
        {step === 1 && (
          <>
            ...
            <form className="mt-10">
              <div className="mt-4 border-0 lg:border border-gray-400 p-2 lg:p-4 rounded-[10px] min-h-[500px]">
                <h4 className="font-semibold text-gray-500 mb-4">Symptom Tracker</h4>

                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-sm">
                  {[
                    "Fever", "Headache", "Fatigue", "Cough", "Stomach pain",
                    "Shortness of breath", "Chest Pain", "Nausea", "Dizziness", "Rash",
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                  ...
                  <div>
                    <p className="font-semibold text-gray-500">Duration (days)</p>
                    <input
                      type="number"
                      placeholder="Number of days"
                      className="w-full border-2 border-gray-400 p-2 rounded-[10px] mt-1"
                  ...
                  <div>
                    <p className="font-semibold text-gray-500">Pain Scale (1-10)</p>
                    <input
                      type="number"
                      min="1"
                      max="10"
                ...
                <div className="mt-4">
                  <p className="font-semibold text-gray-500">Symptom Progression</p>
                  <select
                    className="w-full border-2 border-gray-400 rounded-lg p-2 mt-1"
                    value={formData.coreSymptoms.symptomProgression}
                    onChange={(e) => updateCoreSymptoms({ symptomProgression: e.target.value as 'Getting worse' | 'Staying same' | 'Getting better' | '' })}
                  >
                ...
                <div className="mt-4">
                  <p className="font-semibold text-gray-500">What triggers/worsens symptoms?</p>
                  <textarea
                    className="w-full border-2 border-gray-400 rounded-lg p-2 mt-1"
                    placeholder="e.g., physical activity, certain foods, stress..."
                    value={formData.coreSymptoms.triggeringFactors}
                    onChange={(e) => updateCoreSymptoms({ triggeringFactors: e.target.value })}
                  />
                </div>

                {/* Emergency Symptoms */}
                <div className="my-4 bg-red-50 p-3 rounded-lg">
                  <p className="font-semibold text-red-700 mb-2">⚠️ Emergency Symptoms</p>
        ...
        {step === 2 && (
          <>
            <h2 className="font-semibold text-2xl text-[#005eaa]">Medical History</h2>
            <p className="text-gray-700">Essential background for accurate diagnosis.</p>

            <form className="mt-10">
              <div className="mt-4 border-0 lg:border border-gray-400 p-2 lg:p-4 rounded-[10px] min-h-[500px]">
                <h4 className="font-semibold text-gray-500 mb-4">Existing Medical Conditions</h4>

                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 text-sm">
                  {[
                    "Diabetes", "Hypertension", "Asthma", "Heart Disease", "Stroke",
                    "Kidney Disease", "Cancer", "Epilepsy", "Sickle Cell", "HIV/AIDS",
                ...
                <div className="mt-4">
                  <p className="font-semibold text-gray-500">Other medical conditions</p>
                  <textarea
                    className="w-full border-2 border-gray-400 rounded-lg p-2 mt-1"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  ...
                  <div>
                    <p className="font-semibold text-gray-500">Previous Surgeries</p>
                    <textarea
                      className="w-full border-2 border-gray-400 rounded-lg p-2 mt-1"
                ...
                <div className="my-4 grid grid-cols-1 gap-4 lg:grid-cols-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => updateMedicalHistory({ height: e.target.value ? parseInt(e.target.value) : '' })}
                    />
                  </div>
                </div>

                <div className="my-4">
                  <p className="font-semibold text-gray-500">Last doctor visit</p>
                  <input
                    type="text"
                    className="w-full border-2 border-gray-400 rounded-lg p-2 mt-1"
                    placeholder="e.g., 6 months ago, last week..."
                    value={formData.medicalHistory.lastDoctorVisit}
                    onChange={(e) => updateMedicalHistory({ lastDoctorVisit: e.target.value })}
                  />
                </div>
        ...
        {step === 3 && (
          <>
            <h2 className="font-semibold text-2xl text-[#005eaa]">Supporting Factors</h2>
            <p className="text-gray-700">Lifestyle information for better assessment</p>

            <form className="mt-10">
              <div className="mt-4 border-0 lg:border border-gray-400 p-2 lg:p-4 rounded-[10px] min-h-[500px]">
                <h4 className="font-semibold text-gray-500 mb-4">Lifestyle Assessment</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    <p className="font-semibold text-gray-500">Exercise Level</p>
                    <select
                      className="w-full border-2 border-gray-400 rounded-lg p-2 mt-1"
                      value={formData.supportingFactors.exerciseLevel}
                      onChange={(e) => updateSupportingFactors({ exerciseLevel: e.target.value as 'Sedentary' | 'Light' | 'Moderate' | 'High' })}
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => updateSupportingFactors({ dietQuality: e.target.value as 'Poor' | 'Fair' | 'Good' | 'Excellent' })}
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    <p className="font-semibold text-gray-500">Sleep (hours/night)</p>
                    <input
                      type="number"
                      min="0"
                      max="24"
                      ...
                      onChange={(e) => updateSupportingFactors({ sleepHours: e.target.value ? parseInt(e.target.value) : '' })}
                    />
                  </div>
                  <div>
                    <p className="font-semibold text-gray-500">Stress Level (1-10)</p>
                    <input
                      type="number"
                      min="1"
                      max="10"
                      ...
                      onChange={(e) => updateSupportingFactors({ stressLevel: e.target.value ? parseInt(e.target.value) : '' })}
                    />
                  </div>
                </div>

                <div className="mb-4">
                  <p className="font-semibold text-gray-500">Current Medications</p>
                  <textarea
                    className="w-full border-2 border-gray-400 rounded-lg p-2 mt-1 min-h-[80px]"
                ...
                <div className="mb-4">
                  <p className="font-semibold text-gray-500">Family Medical History</p>
                  <textarea
                    className="w-full border-2 border-gray-400 rounded-lg p-2 mt-1 min-h-[80px]"
                ...
                <div className="mb-8">
                  <p className="font-semibold text-gray-500">Additional Notes</p>
                  <textarea
