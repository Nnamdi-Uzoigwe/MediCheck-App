...
module.exports = (req, res, next) => {
  try {
    ...
    // 4. Attach user to request - FIXED: Use the correct property names from your token
    req.user = { 
      id: decoded.id,           // ✅ Fixed: was decoded.userId, now decoded.id
      email: decoded.email,     // ✅ Added: email from token
      iat: decoded.iat,         // ✅ Added: issued at
      exp: decoded.exp          // ✅ Added: expires at
    };

    console.log("Attached user to request:", req.user);

    // 5. Optional: Add security headers
    res.setHeader('X-Authenticated-User', decoded.id);
